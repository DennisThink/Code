    ...
    std::string strConfig=R"(
{
   ...
   "server":{
	...
	"wechat":{
		"token":"www.dennisthink.com",
		"appid":"www.dennisthink.com",
		"secrect":"www.dennisthink.com",
		"startHour":20，
		"endHour":23
	}
}
...
int main(int argc, char *argv[])
{
	...
	std::error_code ec;
	if (!server->loadConfig(cfg))
	{
		LOG_ERR(logger, "Config Error [{} {}]", __FILENAME__, __LINE__);
		exit(1);
	}
...
int main(int argc, char *argv[])
{
	...
	LOG_WARN(theApp.m_loger,"Server: {}",g_SERVER_VERSION);
      auto worker=[cfg]()->int{     //后台任务
        ...
        if(!server->loadConfig(cfg))
		{
			LOG_ERR(theApp.m_loger, "Config Error [ {} {} ]", __FILENAME__, __LINE__);
			exit(1);
		}
