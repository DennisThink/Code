...
namespace ClientCore
{
	...
	void CHttpServer::Get_Version(std::shared_ptr<HttpServer::Response> response, std::shared_ptr<HttpServer::Request> request) {
		if (request)
		{
			std::string strEchoStr = "Web Site https://www.dennisthink.com Version " + g_SERVER_VERSION;
	...
	/**
	 * @brief 处理公众号服务器转发的用户消息
	 * 
	 * @param strReq 用户消息的XML格式请求
	 * @return std::string 被动回复的用户消息XML格式
	 */
	std::string CHttpServer::HandlePost(const std::string& strReq)
	{
		if (m_pServer)
		{
			return m_pServer->HandlePost(strReq);
		}
		else
		{
			return "";
	...
	/**
	 * @brief 从HTTP的GET请求中,获取strKey对应的value
	 * 
	 * @param request HTTP的请求
	 * @param strKey 参数对应的key
	 * @return std::string key对应的Value
	 */
	...
	void CHttpServer::Init() {
		...
		m_httpServer.resource["/WXChat"]["GET"] = [pSelf, this](std::shared_ptr<HttpServer::Response> response, std::shared_ptr<HttpServer::Request> request) {
			this->Get_Chat(response, request);
		};

		m_httpServer.resource["/WXChat"]["POST"] = [pSelf, this](std::shared_ptr<HttpServer::Response> response, std::shared_ptr<HttpServer::Request> request) {
