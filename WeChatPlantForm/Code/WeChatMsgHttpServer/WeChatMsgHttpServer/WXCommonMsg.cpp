...
WXMsgType ParseMsgType(const std::string& strXml)
{
	...
	if (XMLError::XML_SUCCESS == doc.Parse(strXml.c_str()))
	{
		...
		if ("text" == strType)
		{
			std::string strBizMsgMenuId = GetXMLElemOrgValue(doc, "bizmsgmenuid");
			if (strBizMsgMenuId.empty())
			{
				return WXMsgType::UserSendTxt;
			}
			else
			{
				return WXMsgType::UserSendMenuChoice;
			}
		}
...
/**
 * @brief 用户发送的菜单消息的反序列化
 * 
 * @param strMsg 公众号服务器发送的XML数据
 * @return true 成功
 * @return false 失败
 */
bool UserSendMenuChoiceMsg::FromString(const std::string strMsg)
{
	XMLDocument doc;
	if (XMLError::XML_SUCCESS == doc.Parse(strMsg.c_str()))
	{
		...
		m_strContent = GetXMLElem_CDATA_Value(doc, "Content");
		m_strMsgId = GetXMLElemOrgValue(doc, "MsgId");
		m_strBizMsgMenuId = GetXMLElemOrgValue(doc, "bizmsgmenuid");
		return true;
	}
	else
	{
...
std::string UserSendMenuChoiceMsg::ToString()
{
	return "";
}

WXMsgType UserSendMenuChoiceMsg::getMsgType()
{
	return WXMsgType::UserSendMenuChoice;
}
