...
void Do_UserLogin(HttpClient& client) {
	...
	try {
		...
		reqMsg.m_strPassword = "srpPr9MXwp285Su";
...
void Do_GetEmail(HttpClient& client) {
	GetEmailRsp rspMsg;
	try {
		auto rsp = client.request("GET", "/get_email?UserId=dennismi@163.com");
		std::string strRsp = rsp->content.string();
		std::cout << strRsp << std::endl;
		CHECK(rspMsg.FromString(strRsp));
...
void GetEmail(HttpClient& client) {
	Do_GetEmail(client);
}


//TEST_CASE("SMTP_SendEmail") {
