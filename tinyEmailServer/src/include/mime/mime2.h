...
#include "../common/util/ProtoUtil.h"
#include <string>
#include <vector>
#include <sstream>
#include <cstring>
...
{
	return cpp_email::CProtoUtil::Base64Encode(std::string(Z, s));
}

inline void Base64ToChar(const char *Z, size_t s, vector<char> &out)
{
	std::string strResult = cpp_email::CProtoUtil::Base64Decode(std::string(Z, s));
	for (std::size_t i = 0; i < strResult.length(); i++)
	{
		out.push_back(strResult[i]);
	}
}
...
/**
 * @brief 将m中的字符串按照del切成string的数组，放到result中
 * 
 * @param m 待切分的字符串
 * @param del 用来切分的字符
 * @param result 切分的结果
...
class CONTENT
{
	...
	char *QPDecode(char *input)
	{
		...
		while (*s != '\0') //loop through the entire string...
		{
			if (*s == '=') //woops, needs to be decoded...
			{
				...
				for (int i = 0; i < 2; i++)
				{
					if (hexmap[static_cast<int>(s[i])] == SKIP)
					{
				...
				{
					s += 2;
					int m = hexmap[static_cast<int>(mid[0])];
					m <<= 4;
					m |= hexmap[static_cast<int>(mid[1])];
					*(result++) = (char)m;
				}
			}
			else
  ...
  public:
	...
	{
		if (ss == static_cast<std::size_t>(-1))
			ss = strlen(a);
		else
		{
			vector<char> d(ss);
	...
	void DecodeData(vector<char> &d)
	{
		...
		if (strcasecmp(a2.c_str(), "base64") == 0)
		{
			/*DWORD dw = 0;
		...
		if (strcasecmp(a2.c_str(), "quoted-printable") == 0)
		{
			vector<char> nd(data.size() + 10);
			strncpy(nd.data(), data.data(), nd.size());
			char *ce = QPDecode(nd.data());
			d.resize(strlen(ce) + 1);
			strncpy(d.data(), ce, d.size());
			free(ce);
			return;
		}
		d = data;
